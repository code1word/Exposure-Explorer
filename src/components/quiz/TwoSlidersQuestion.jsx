...
function TwoSlidersQuestion({ info, questionKey, reviewMode = false }) {
  ...
  const imageIndex = Math.round(sliderValue * 10); // 3, 6, 9
        ...
        <Row className="align-items-start">
          ...
          <QuizPhotoSlider
            ...
            imageSrcFunction={(val) => {
              ...
              const index = Math.round(clamped * 10); // 3, 6, 9
              return portraitImageMap[index];
            }}
            min={0.3}
            max={0.9}
            step={0.3} // Only 0.3, 0.6, 0.9 allowed
...
export default TwoSlidersQuestion;
